...
targets <- list(
  ...
  list(kind = "dir_contents", path = here("data", "derived", "qc"), note = "QC regenerable"),
  list(kind = "dir_contents", path = here("outputs"), note = "salidas auxiliares regenerables"),
  list(kind = "dir_contents", path = here("reports", "qc_demografia_poblacion"), note = "portal QC regenerable"),
  list(kind = "dir_filtered", path = here("reports"), note = "renders regenerables; preserva QMD y JS/CSS fuente", keep_pattern = "\\.(qmd|css|js)$"),
  list(kind = "dir_contents", path = here("data", "_catalog"), note = "catalogos regenerables")
)
